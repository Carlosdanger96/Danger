...
4. **Quantize the model (GGUF)**
   ...
   llm_integration/llama.cpp/quantize \
       mistral-f16.gguf mistral-q4.gguf q4_K_M
   ```
   The `q4_K_M` quantization yields a good trade‑off between quality and size.
   The output file `mistral-q4.gguf` is around 4 GB.
...
5. *(Optional)* **Quantize with AWQ**
   ...
   ```bash
   python3 llm_integration/quantize_mistral_awq.py Mistral-7B-v0.1 awq_out
   ```
   The resulting directory `awq_out/` contains AWQ weights that compatible
   runtimes can load.
